...
#SBATCH --job-name=yhr_cross_yolov3-tiny-qt_pr
...
parser.add_argument("--thetas_lr", type=float, default=0.1,
                    help="learning rate for architecture para.")
parser.add_argument("--scale_sparse_rate", type=float, default=5e-5,
                    help="Scale sparse rate for pruning")
parser.add_argument("--start_quant", type=int, default=3,
                    help="Set the epoch from which start quantizing the model")
parser.add_argument("--train_thetas_from_the_epoch", type=int, default=20,
...
parser.add_argument("--current_save_path", type=str,                                                                           default='./checkpoints/current_model.pth/current_model.pth',
                    help="Save the best model path")
parser.add_argument('--train_or_sample', type=str, default='sample',
...
def train_supernet(resume=False):
    ...
    w_criterion = SupernetLoss().cuda()
    theta_criterion = SupernetLoss().cuda()
    ...
    #### Training Loop
    trainer = TrainerSupernet(w_criterion, theta_criterion, w_optimizer, theta_optimizer, w_scheduler,
                              args.scale_sparse_rate, args.start_quant, args.train_thetas_from_the_epoch, 
                              logger, writer)
...
def sample_architecture_from_the_supernet(unique_name_of_arch):
    ...
    checkpoint = torch.load(CONFIG_SUPERNET['train_settings']['path_to_save_current_model'])
    model.load_state_dict(checkpoint["state_dict"])
    ...
    for i, layer in enumerate(model.module_list[2:13]):
        if i % 2 == 0:          
            arch_operations_backbone.append(ops_names_backbone[np.argmax(layer.AP_path_alpha.detach().cpu().numpy())])
            
    layer26, layer13 = model.module_list[16], model.module_list[17]
