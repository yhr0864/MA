...
#SBATCH --job-name=yhr_sampled_net_cross_yolov3
...
def main():
    ...
    # print(sub_model.state_dict().keys())
...
#     globals()['op'], globals()['input_sample'] = sub_model.cpu(), input
#     total_time = timeit.timeit('output = op(input_sample)', setup="gc.enable()",
#                                globals=globals(), number=1)
#     macs, params = profile(sub_model, inputs=(input,))
#     print(total_time)
#     print(macs, params)
#     # print(out.shape)
#     print(out[0].shape)
...
#     checkpoint = torch.load(CONFIG_ARCH['sub-model-saving'])
#     sub_model.load_state_dict(checkpoint)
#
#     bn_weights = gather_bn_weights(sub_model)
#     sorted_bn = torch.sort(bn_weights)[0]
#     print(sorted_bn.numpy())
#     ns, edgeBin, patches = plt.hist(sorted_bn.numpy(), bins=20, rwidth=0.8)
#     plt.show()
# #     # 接下来可以剪枝
# #     ##############
# #     #第一步先BN同步
...
#                                            CONFIG_ARCH['dataloading']['img_size'],
#                                            percent=percent_limit-0.4) # 0.1~0.2
#
...
#
#     #### Loss and Optimizer
#     # optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, sub_model.parameters()),
#     #                             lr=CONFIG_ARCH['optimizer']['lr'],
#     #                             momentum=CONFIG_ARCH['optimizer']['momentum'],
#     #                             weight_decay=CONFIG_ARCH['optimizer']['weight_decay'])
#     optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, sub_model.parameters()),
#                                  lr=CONFIG_ARCH['optimizer']['lr'],
#                                  weight_decay=CONFIG_ARCH['optimizer']['weight_decay'])
#     criterion = Loss().cuda()
#
...
if __name__ == "__main__":
    # main()
    #
    # res = 3*3*3*16 + 7*7*16*32 + 32*51 + 5*5*51*118 + 9*118*170 + 49*170*179 + 9*179*139 + 139*78 + 78*22 + 49*(170+22)*93 + 25*78*99 + 93*27 + 99*27 + 54 + 2226
    # res2 = 3 * 3 * 3 * 16 + 9 * 16 * 32 + 32 * 51 + 9 * 51 * 118 + 9 * 118 * 170 + 9 * 170 * 179 + 9 * 179 * 139 + 139 * 78 + 78 * 22 + 9 * (
    #             170 + 22) * 93 + 9 * 78 * 99 + 93 * 27 + 99 * 27 + 54 + 2226
    # print(res)
    # print(res2)
    ...
    print(sigmoid_function([2.38,1.19,]))
    #input = torch.randn(1, 3, 416, 416)
    #out = model(input)
    #
    #print(out[0].shape)
