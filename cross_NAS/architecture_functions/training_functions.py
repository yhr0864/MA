...
class TrainerArch:
    ...
    def train_loop(self, train_loader, valid_loader, model):
        ...
        for epoch in range(self.cnt_epochs):
            ...
            # }
            torch.save(model, self.path_to_save_current_model) # save the supernet

            if mAP is not None:
                self.logger.info("best mAP is : %f" % best_mAP)
                self.logger.info("current mAP is : %f" % mAP)
                if best_mAP < mAP:
                    best_mAP = mAP
                    # self.logger.info("Best mAP by now")
                    self.logger.info("Best mAP by now. Save model")
                    torch.save(model, self.path_to_save_model)
