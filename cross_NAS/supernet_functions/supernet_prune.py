...
class PrunedModel:
    ...
    def get_highest_thre(model):
        ...
        print(f'The corresponding prune ratio limit is {percent_limit:.3f}.')
    ...
    def prune_and_eval(model, test_loader, img_size, percent=.0):
        ...
        print(f'Channels with Gamma value less than {thre:.6f} are pruned!')
        mAP_origin = eval_model(model_copy, test_loader, img_size)
        print(f'mAP of the origin model is {mAP_origin:.5f}')
        
        remain_num = 0

        for idx, m in enumerate(model_copy.module_list):
            if 2 <= idx <= 17 and isinstance(m, ConvBNRelu):
                bn_module = m[1]
        ...
        print(f'Prune ratio in theory: {1-remain_num/len(sorted_bn):.3f} (setup manually!)')
        print(f'mAP of the pseudo-pruned model is {mAP:.4f}')
    ...
    def obtain_filters_mask(model, thre):
        ...
        for idx, module in enumerate(model.module_list):
            if 2 <= idx <= 17:
                ...
                if idx == 0 and isinstance(module, ConvBNRelu):
                    bn_module = module[1]
        ...
        print(f'Prune channels: {pruned}\tPrune ratio in real: {prune_ratio:.3f}')
